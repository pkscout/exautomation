...
Destinations can merge files from a given source into one file (in case the destination only accepts one file).  You set that for a destination by using `'mergfiles': True` in the destination settings.  By default the script assumes all files have a header row and doesn't duplicate it.  If a given source doesn't have a header row in the file, you need to add `'hasheaders':{'<source>': False, '<source2>': False}` to the destination's settings as well.  If a given source is not in the dictionary, then the script assumes that source has a header row (so basically you only need to add a source to the `hasheaders` section if you're going to set that source to `False`).
